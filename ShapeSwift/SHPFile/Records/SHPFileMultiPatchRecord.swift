...
extension SHPFileMultiPatchRecord {
  private struct Parser {
    ...
    init(data: Data, start: Int) throws {
      ...
      self.partTypes = ByteParser<MultiPatchPartType, LittleEndian>(start: parts.end, count: numParts)
      self.points = ByteParser<Coordinate2D, LittleEndian>(start: partTypes.end, count: numPoints)
...
extension SHPFileMultiPatchRecord: SHPFileRecord {
  ...
  init(recordNumber: Int, data: Data, range: Range<Int>, endByte: inout Int) throws {
    ...
    self.partTypes = try parser.partTypes.parse(data)
